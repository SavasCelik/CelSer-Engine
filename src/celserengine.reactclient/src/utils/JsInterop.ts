...
class JsInterop {
  ...
  attachDotNetObjectAsync(
    dotNetTypeName: string,
    componentId: string
  ): Promise<number> {
    return this.invokeDotNetMethodAsync<number>(
      null,
      "AttachDotNetObject",
      JSON.stringify([dotNetTypeName, componentId])
    );
  }

  detachDotNetObjectAsync(dotNetRefId: number): Promise<unknown> {
    return this.invokeDotNetMethodAsync(
      null,
      "DetachDotNetObject",
      JSON.stringify(dotNetRefId)
    );
  }
  ...
  invokeComponentMethod(
    ...
    if (typeof method === "function") {
      ...
      throw new Error(
        `Method ${methodName} is not defined on component with ID ${componentId}.`
      );
    }
  }
