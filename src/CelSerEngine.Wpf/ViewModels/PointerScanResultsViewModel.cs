...
public partial class PointerScanResultsViewModel : ObservableRecipient
{
    ...
    public async Task StartPointerScanAsync(PointerScanOptions pointerScanOptions)
    {
        ...
        FoundPointers = await pointerScanner.StartPointerScanAsync(selectedProcess.GetProcessHandle(_nativeApi), cancellationToken: _currentCts.Token);
    ...
    public async Task RescanScan(string nextAddress)
    {
        ...
        var pointerScanner = new DefaultPointerScanner((NativeApi)_nativeApi, new PointerScanOptions());
        var foundPointers = await pointerScanner.RescanPointersAsync(FoundPointers, searchedAddress, processHandle);
        FoundPointers = foundPointers;
    }
